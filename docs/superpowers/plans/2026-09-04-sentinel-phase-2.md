...
| `ingest/storage.py` | Parquet partition read/write, part-file discovery, source-tree removal; `read_corpus` is a partition reader, not a corpus reader (D27) | Create |
| `ingest/manifest.py` | `CorpusManifest`, checksums, atomic manifest write, `load_corpus` — the only corpus reader (D27) | Create |
...
load   → data/corpus/<source>/v<N>/year=<YYYY>/part-NNNN.parquet, manifest.json last (replaces the tree)
```

Interrupting `fetch` loses at most one page. Re-running `fetch` skips pages already on disk (verified by checksum, not existence). `normalize` is deterministic given the same raw pages, and `load` replaces the source's whole versioned tree rather than adding to it — deleting the manifest first and writing it last (addendum §2.7, D27) — so the corpus is reproducible from the raw cache without re-hitting an API. A load that fails leaves no manifest and is recovered by rerunning it.
...
`manifest.json` per source records: `manifest_version`, `schema_version`, `source_slug`, `window_start`/`window_end` (the **resolved instants**, not the dates supplied — see addendum §2.5), `ingested_at`, `record_count`, `per_year_counts`, the observed label roster with per-label counts — **describing the persisted corpus**, which on a `--limit`ed run need not hold every label the window contained (D24) — `part_files` with a SHA256 per file, the adapter's `source_api_version` string, `corpus_id` — a stable hash over the part-file checksums that artifacts cite to name exactly which corpus they trained on — plus `limit` and `timestamp_diagnostic`.

**The manifest is the validity boundary (D27, addendum §2.7).** A manifest describes exactly the part files of the completed run that wrote it: a successful run replaces the source's whole `v<N>` tree with its current window, deleting the manifest before anything else and writing it last through a same-directory temporary file and `os.replace`. A source/version tree without a valid manifest holds no corpus, whatever Parquet files it contains. Every consumer reads a corpus through `load_corpus`, which requires the manifest, rejects unlisted part files, verifies every listed file's bytes, and streams only those files; `read_corpus` is a partition reader for `build_manifest` and tests only. This is a validity boundary, not atomic directory replacement: a failed run loses the previous corpus, and while a run is between deleting the old manifest and writing the new one the source has no corpus.
...
**`limit`** — a positive `int`, or `None`. The `--limit` passed to the ingestion CLI, or `null` for an unbounded run; zero and negative values are refused with `InvalidLimit` before anything is read or written (D28, addendum §2.8). On a limited manifest `1 <= record_count <= limit` always holds (D23, D27, D28). Recorded so a truncated development corpus can never be mistaken for a full one; every artifact citing a `corpus_id` can see whether the corpus behind it was complete. Since D26 that guarantee is enforced, not merely recorded: a `--limit`ed run refuses to write into a corpus root that already holds an authoritative (`limit: null`) corpus, so a truncated corpus can never take a full one's place (addendum §2.6).

`limit` bounds **persistence only** (D24). Roster derivation and assertion run over the complete normalized window before any truncation, so a development flag can never make a source's taxonomy look incomplete. The field is also what makes a truncated manifest non-authoritative: a manifest whose `limit` is non-null is never adopted as the roster lock, and a run reads its lock only from its own target corpus root. D26 additionally prevents a truncated manifest from ever displacing an authoritative one: a limited run into a root holding an authoritative corpus is refused before any write. A limited run therefore never consults an authoritative roster; it derives its roster from its own complete window, and D26 takes precedence over D24's original same-root limited-run case. That is this field earning the justification the paragraph above already gives it, rather than merely stating it.
...
Seeds are set explicitly for numpy, scikit-learn estimators and the perturbation sampler, and recorded in metadata. Corpus ordering is deterministic (sorted by `(submitted_at, external_id)`) before any split. `corpus_id` ties an artifact to exact input bytes, because every experiment reads its corpus through `load_corpus`, which verifies each part file against the manifest before yielding a record (addendum §2.7, D27). `dependency_versions` records the installed versions of numpy, scikit-learn and onnxruntime.
...
**Behavior:** `write_partition(records, source, year, part_index) -> Path`; `read_corpus(source, years=None) -> Iterator[CorpusRecord]` yielding in deterministic `(submitted_at, external_id)` order — a **low-level partition reader** that reads whatever part files exist, remains usable before any manifest exists, asserts nothing about validity, and is used only by `build_manifest` and by tests (D27); `CorpusManifest` with the §G fields; `compute_corpus_id(part_checksums) -> str` as a SHA256 over sorted `(path, sha256)` pairs. D27 adds three things. Removal of one source's `v<SCHEMA_VERSION>` tree, deleting its manifest before anything else. `write_manifest` writing a same-directory temporary file moved into place with `os.replace`. And `load_corpus(source, years=None) -> tuple[CorpusManifest, Iterator[CorpusRecord]]`, the **validity-gated production reader** and the only corpus reader: it requires the manifest (`ManifestNotFound`), rejects a part file on disk that `part_files` does not list (`CorpusIntegrityError`), verifies every listed file's bytes (`ChecksumMismatch` when missing or altered), and streams only the listed files in the same order (addendum §2.7).
**Tests:** a written-then-read corpus round-trips records exactly; read order is deterministic across two calls and independent of file order on disk; `corpus_id` changes when a part file's bytes change and is stable when only mtime changes; a manifest whose checksum does not match its part file raises; **`read_corpus` reads partitions for which no manifest exists yet**; **`load_corpus` raises `ManifestNotFound` without a manifest, a `CorpusIntegrityError` for an unlisted part file, and `ChecksumMismatch` for a missing or altered listed file**; **`load_corpus` yields exactly the listed files' records in `(submitted_at, external_id)` order**; **tree removal deletes the manifest first and touches no other source and no other schema version**; **a failure injected during `write_manifest` leaves no partial `manifest.json`**; **no module under `ingest/` or `ml/` other than `ingest/manifest.py` imports `read_corpus`**.
**Acceptance:** `read_corpus` never loads more than one part file into memory at a time (asserted by reading a two-part corpus with a memory-bounded assertion on chunk size). The same bound holds for `load_corpus`.
...
**Behavior:** `python -m ingest.cli --source {cfpb,nyc311} --start YYYY-MM-DD --end YYYY-MM-DD [--limit N] [--corpus-root PATH]`. Fetch writes gzipped pages under `data/raw/`; a page whose checksum matches an existing file is skipped. Normalize and load are re-run from the raw cache without network access. A successful run replaces the source's whole `v<N>` tree with the current window, even a narrower one: after every pre-write refusal has passed it deletes the manifest, then the rest of the tree, writes the partitions, and writes the manifest last through a same-directory temporary file and `os.replace`. A run that fails after clearing leaves no manifest, so the root holds no corpus, and is recovered by rerunning from the raw cache (D27, addendum §2.7). `--limit` bounds **the records persisted** for development and is recorded in the manifest so a truncated corpus can never be mistaken for a full one. It does not bound the roster population: derivation and assertion run over the complete normalized window before truncation; a manifest whose `limit` is non-null never becomes the roster lock, and the lock is read only from the run's own target root (D24). `--corpus-root PATH` names the corpus root written to, defaulting to the existing corpus root (`data/corpus/`). A `--limit`ed run whose target root already holds an authoritative manifest (`limit: null`) raises `AuthoritativeCorpusExists`, a subclass of `IngestError`, before `fetch_into_cache` and before any write; the message states the source, the corpus root, the existing `corpus_id` and `record_count`, and that a limited run must target a different root. A limited run into a fresh root or into a root holding a truncated corpus is permitted, and an unbounded rerun over an authoritative corpus remains permitted and idempotent (D26, addendum §2.6). `--limit` must be a positive integer: `0` or a negative value raises `InvalidLimit`, a subclass of `IngestError`, immediately after the `InvalidDateRange` check and before the D26 refusal, any fetch, or any write (D28, addendum §2.8). `--start` and `--end` are inclusive whole civil days, resolved in `America/New_York` for 311 and in UTC for CFPB, then compared as UTC instants (D25, addendum §2.5). A window that normalizes to zero records raises `EmptyWindow` before roster derivation and writes neither a partition nor a manifest (D23).
...
**Tests:** with a stub adapter, an interrupted run resumes without duplicating records; re-running with the raw cache present performs zero fetches; `--limit` is recorded in the manifest; the roster assertion from Task 7 runs before any Parquet file is written (asserted by checking no part file exists after a `RosterMismatch`); **a synthetic CFPB corpus with a 3-second median delta yields `strongly_suspicious_load_timestamp`; one with a multi-hour median and no sub-minute mass yields `supported_plausible_event_time`; one with 40% coverage yields `suspicious_insufficient_evidence` regardless of its deltas**; **a corpus with a uniform hour histogram but healthy multi-hour deltas does NOT yield `strongly_suspicious_load_timestamp`** (distribution alone cannot establish artifact status); a corpus with extreme hour concentration downgrades `supported` to `suspicious_insufficient_evidence` but never further; 311 is recorded `suspicious_insufficient_evidence` with `not_directly_testable: true`; both parts carry their `evidence_class`; the rule thresholds appear in the manifest; **a window normalizing to zero records raises `EmptyWindow` and leaves no partition and no manifest**; **on a permitted limited run into a fresh root, roster validation covers the complete normalized window before truncation: a label present in only one year of the window still raises `RosterMismatch` even when `--limit` would have dropped every record carrying it**; **a manifest written with a non-null `limit` is not adopted as the roster lock by a later run**; **a 311 record at 20:00 New York time on the window's last day is inside the window, and the same wall clock read as UTC would place it outside**; **`--end` includes its own whole day**; **a `--limit`ed run into a root holding an authoritative corpus raises `AuthoritativeCorpusExists` before any fetch, leaving every partition byte-identical and the manifest's `corpus_id` unchanged**; **a `--limit`ed run into a fresh root succeeds and records its `limit`**; **a `--limit`ed run into a root holding a truncated corpus succeeds**; **an unbounded rerun over an authoritative corpus succeeds with an unchanged `corpus_id`**; **`--corpus-root` reaches `ingest()` from the command line and defaults to the existing corpus root when omitted**; **a limited run into a truncated root whose earlier corpus covered more years leaves no stale partition, and `record_count <= limit`**; **an unbounded run over a narrower window than an authoritative corpus removes every out-of-window partition, and its manifest's window, counts and `corpus_id` agree with disk**; **after any successful run the part files on disk equal `part_files` and `load_corpus` succeeds**; **a run made to fail after its first partition write leaves no manifest, `load_corpus` raises `ManifestNotFound`, the root is not authoritative, and a rerun recovers a consistent corpus**; **every pre-write refusal leaves an existing corpus byte-identical**; **`--limit 0` and `--limit -1` raise `InvalidLimit` through both `ingest()` and the command line with no manifest read, no fetch and no write, while `--limit 1` succeeds**.
**Acceptance:** no test performs a network call; CLI is importable without Django; `timestamp_diagnostic` present in every manifest with both evidence classes labelled and the verdict rule recorded; no `--limit`ed run can leave an authoritative corpus modified; after every successful run the corpus on disk is exactly what its manifest lists; a failed run never leaves a manifest describing bytes it did not write; no `--limit` below 1 is accepted.
...
**Behavior:** Loads the CFPB corpus through `load_corpus` (addendum §2.7), applies `temporal_split`, fits TF-IDF on train text only, trains a calibrated linear classifier, tunes the abstention threshold on validation, evaluates on test, and writes an artifact per §P.
...
**Behavior:** Loads the 311 corpus through `load_corpus` (addendum §2.7), splits, fits thresholds on train, builds out-of-fold aggregates for train and frozen aggregates for val/test, trains `HistGradientBoostingClassifier`, tunes banding on validation, evaluates on test, writes the artifact.
...
**Behavior:** Trains a separate `HistGradientBoostingClassifier` on the 311 corpus, loaded through `load_corpus` (addendum §2.7), using `TRANSFER_FEATURES_V1` only, targeting `nyc311_sla_breach` (thresholds from Task 13, frozen). Evaluates in-domain on held-out 311 and cross-domain/cross-target on the CFPB corpus, likewise loaded through `load_corpus`, against `cfpb_timely_response`. Writes its own artifact with `model_name="xdomain_xtarget_probe"`, its own `model_version`, `feature_spec` of exactly the three names, and `experiment_label="reduced-feature cross-domain cross-target robustness probe"`.
...
Each task is one commit, so rollback is `git revert` of that commit. The corpus is regenerable from `data/raw/` without network access, and from the source APIs given the recorded window. A load that fails after clearing the source's tree leaves no manifest, so `load_corpus` refuses the root rather than reading a partial corpus; recovery is rerunning the ingest from the raw cache (addendum §2.7, D27). The previous corpus is not preserved across that failure. Artifacts are versioned directories, so a bad model is abandoned by not pinning it — nothing in Phase 2 is wired into serving, so no rollback touches the running application. If a leakage test fails after a later change, the responsible task is identified from the failing property and corrected with a regression test rather than patched around.
...
| A successful run replaces the source's tree; no stale partition; a narrower window replaces completely (D27) | 8 | `test_cli` shrinking-years and narrower-window replacement tests | Covered |
| No manifest, no corpus; `load_corpus` is the only corpus reader (D27) | 4, 8 | `test_manifest` gated-reader refusals; `read_corpus` import guard; `test_cli` failed run leaves no manifest | Covered |
| `--limit` is a positive integer (D28) | 8 | `test_cli` `InvalidLimit` for `0` and `-1` | Covered |
