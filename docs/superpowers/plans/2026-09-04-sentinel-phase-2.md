...
**M2 — `mypy` targets do not include `ml/training/`.** CI runs `mypy complaints domains ml accounts`; since `ml/training` is under `ml`, it is covered, but `ingest` is not. Task 1 adds `ingest` to the CI mypy target.
