...
**Acceptance:** no test performs a network call; CLI is importable without Django; `timestamp_diagnostic` present in every manifest with both evidence classes labelled and the verdict rule recorded; no `--limit`ed run can leave an authoritative corpus modified.
